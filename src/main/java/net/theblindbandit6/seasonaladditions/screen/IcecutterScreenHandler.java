...
import net.theblindbandit6.seasonaladditions.recipe.ModRecipeManager;
...
public class IcecutterScreenHandler extends ScreenHandler {
	...
	private CuttingRecipeDisplay.Grouping<IcecuttingRecipe> availableRecipes = CuttingRecipeDisplay.Grouping.empty();
	...
	public CuttingRecipeDisplay.Grouping<IcecuttingRecipe> getAvailableRecipes() {
	...
	private void updateInput(ItemStack stack) {
		...
		if (!stack.isEmpty()) {
			this.availableRecipes = ModRecipeManager.getIcecutterRecipes().filter(stack); // Fix this
		} else {
	...
	void populateResult(int selectedId) {
		Optional<RecipeEntry<IcecuttingRecipe>> optional;
		if (!this.availableRecipes.isEmpty() && this.isInBounds(selectedId)) {
			CuttingRecipeDisplay.GroupEntry<IcecuttingRecipe> groupEntry = (CuttingRecipeDisplay.GroupEntry<IcecuttingRecipe>)this.availableRecipes
				.entries()
				.get(selectedId);
		...
		optional.ifPresentOrElse(
			recipe -> {
				...
				this.outputSlot
					.setStackNoCallbacks(((IcecuttingRecipe)recipe.value()).craft(new SingleStackRecipeInput(this.input.getStack(0)), this.world.getRegistryManager()));
			},
			() -> {
				this.outputSlot.setStackNoCallbacks(ItemStack.EMPTY);
	...
	public ItemStack quickMove(PlayerEntity player, int slot) {
		...
		if (slot2 != null && slot2.hasStack()) {
			...
			} else if (ModRecipeManager.getIcecutterRecipes().contains(itemStack2)) {
